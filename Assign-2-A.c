//Assignment II-A-> Addition of two polynomials using Linked List 
#include<stdio.h>
#include<stdlib.h>
struct node {
    float coeff;
...
void polyAdd(struct node* head1, struct node* head2)
{
    ...
    struct node *ResultHead = NULL; // Resulted  Linl-list Polynomil Equation

    while (ptr1!=NULL && ptr2!=NULL)
    {
        if(ptr1->expo == ptr2->expo)
        {
            ResultHead = insert(ResultHead, ptr1->coeff+ptr2->coeff, ptr1->expo);
            ptr1 = ptr1->link;
            ptr2 = ptr2->link;
        }
        else if(ptr1->expo > ptr2->expo)
        {
            ResultHead = insert(ResultHead, ptr1->coeff , ptr1->expo);
            ptr1 = ptr1->link;
        }
        else if(ptr1->expo < ptr2->expo)
        {
            ResultHead = insert(ResultHead, ptr2->coeff , ptr2->expo);
            ptr2 = ptr2->link;
        }
    }

    // Now this peace of code will display the remaning terms
        while (ptr1!=NULL)
        {
            ResultHead = insert(ResultHead, ptr1->coeff, ptr1->expo);
            ptr1 = ptr1->link;
